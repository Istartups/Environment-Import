...
import {
  ...
  ChevronDown, MoreHorizontal, MessageSquareText
} from "lucide-react";
    ...
    <div className="max-w-2xl mx-auto px-4 pb-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-5">
        {[
          ...
          { label: "Measurements", value: totalMeasurements ?? "—", icon: Ruler, color: "text-emerald-400", bg: "bg-emerald-500/10" },
      ...
      {quickAccessTools.length > 0 && (
        <section className="mb-5">
          <div className="flex items-center justify-between mb-2.5 px-0.5">
            ...
            <button onClick={() => handleNavigate("/all-tools")} className="text-[10px] font-black uppercase tracking-widest text-primary hover:text-primary/80 transition-colors">
          ...
          <div className="grid grid-cols-4 gap-2">
            {quickAccessTools.map((id) => {
              ...
              return (
                ...
                >
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background: tool.iconBg }}>
      ...
      <section className="mb-8">
        <div className="flex items-center justify-between mb-3 px-1">
          ...
          <button onClick={() => handleNavigate("/add-customer")} className="text-[10px] font-black uppercase tracking-widest text-primary hover:text-primary/80 transition-colors">
            View All <ChevronRight size={10} className="inline ml-0.5" />
          </button>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5">
          <button onClick={() => handleNavigate("/add-customer")} className="group flex md:flex-row flex-col items-center gap-2.5 md:gap-3 p-4 rounded-2xl bg-gradient-to-br from-blue-600/10 to-blue-600/[0.02] border border-blue-500/20 active:scale-[0.97] transition-all">
            <div className="w-10 h-10 rounded-xl bg-blue-600 text-white flex items-center justify-center shadow-lg shadow-blue-600/20 group-hover:scale-110 transition-transform shrink-0">
              <Users size={19} />
            </div>
          ...
          <button onClick={() => setShowMeasureModal(true)} className="group flex md:flex-row flex-col items-center gap-2.5 md:gap-3 p-4 rounded-2xl bg-gradient-to-br from-emerald-600/10 to-emerald-600/[0.02] border border-emerald-500/20 active:scale-[0.97] transition-all">
            <div className="w-10 h-10 rounded-xl bg-emerald-600 text-white flex items-center justify-center shadow-lg shadow-emerald-600/20 group-hover:scale-110 transition-transform shrink-0">
              <Ruler size={19} />
            </div>
            <div className="text-center md:text-left">
              <p className="text-[12px] font-black text-foreground leading-tight">Measure</p>
              <p className="text-[9px] text-muted-foreground font-bold uppercase tracking-tight mt-0.5">Add Quickly</p>
            </div>
          </button>
          <button onClick={() => handleNavigate("/measurement-card")} className="group flex md:flex-row flex-col items-center gap-2.5 md:gap-3 p-4 rounded-2xl bg-gradient-to-br from-amber-600/10 to-amber-600/[0.02] border border-amber-500/20 active:scale-[0.97] transition-all">
            <div className="w-10 h-10 rounded-xl bg-amber-600 text-white flex items-center justify-center shadow-lg shadow-amber-600/20 group-hover:scale-110 transition-transform shrink-0">
              <LayoutGrid size={19} />
            </div>
            <div className="text-center md:text-left">
          ...
          <button onClick={() => handleNavigate("/message-center")} className="group flex md:flex-row flex-col items-center gap-2.5 md:gap-3 p-4 rounded-2xl bg-gradient-to-br from-violet-600/10 to-violet-600/[0.02] border border-violet-500/20 active:scale-[0.97] transition-all">
            <div className="w-10 h-10 rounded-xl bg-violet-600 text-white flex items-center justify-center shadow-lg shadow-violet-600/20 group-hover:scale-110 transition-transform shrink-0">
              <MessageSquareText size={19} />
            </div>
            <div className="text-center md:text-left">
              <p className="text-[12px] font-black text-foreground leading-tight">Follow Up</p>
              <p className="text-[9px] text-muted-foreground font-bold uppercase tracking-tight mt-0.5">Message Center</p>
            </div>
          </button>
        </div>
      </section>
        ...
        <section className="mb-8">
          ...
          {recentCategories.length > 1 && (
            <div className="flex gap-2 overflow-x-auto pb-2 mb-3 no-scrollbar">
              <button onClick={() => setHomeCategoryFilter("all")} className={`shrink-0 px-3 py-1.5 rounded-xl text-[10px] font-bold border transition-all ${homeCategoryFilter === "all" ? "bg-primary/10 border-primary text-primary" : "bg-card border-border text-muted-foreground"}`}>All</button>
              {recentCategories.map(cat => (
                <button key={cat} onClick={() => setHomeCategoryFilter(cat)} className={`shrink-0 px-3 py-1.5 rounded-xl text-[10px] font-bold border transition-all whitespace-nowrap ${homeCategoryFilter === cat ? "bg-primary/10 border-primary text-primary" : "bg-card border-border text-muted-foreground"}`}>{cat}</button>
              ))}
            </div>
          )}
          <div className="space-y-2">
            {filteredRecentMeasurements.map(m => (
              <button key={m.id} onClick={() => handleNavigate(`/add-customer?action=new_measurement&customerId=${m.customerId}`)} className="w-full flex items-center gap-3 p-3.5 rounded-2xl bg-card border border-border hover:border-primary/20 active:scale-[0.98] transition-all text-left">
                <div className="w-9 h-9 rounded-xl bg-primary/10 flex items-center justify-center text-primary shrink-0"><Ruler size={15} /></div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-bold truncate">{m.label}</p>
                    <span className="shrink-0 text-[9px] font-bold px-1.5 py-0.5 rounded-md bg-primary/10 text-primary border border-primary/10">{m.category}</span>
                  </div>
                  <p className="text-[10px] text-muted-foreground mt-0.5">{m.customerName ?? "Unknown client"} · {relativeDate(m.createdAt)}</p>
                </div>
          ...
          {CATEGORIES.map(({ id, emoji }) => {
            ...
            return (
              <button key={id} onClick={() => handleNavigate(`/all-tools?cat=${id}`)}
                className="flex md:flex-row flex-col items-center gap-2 md:gap-3 p-3 md:p-3.5 rounded-2xl text-left active:scale-[0.97] transition-all"
                style={{ background: "hsl(218,44%,11%)", border: "1px solid hsl(218,38%,18%)" }}>
                <span className="text-xl md:text-2xl shrink-0">{emoji}</span>
                <div className="text-center md:text-left min-w-0">
                  <p className="font-bold text-xs md:text-sm capitalize truncate" style={{ color: "hsl(43,25%,88%)" }}>{id}</p>
                  <p className="text-[9px] md:text-xs text-muted-foreground">{count} tools</p>
                </div>
              </button>
            );
          })}
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {displayedFavorites.map((id) => {
              const tool = getToolById(id)!;
              return (
                <button key={id} onClick={() => handleNavigate(tool.path)} className="p-4 rounded-2xl bg-card border border-border flex flex-col gap-3 active:scale-[0.98] transition-all text-left">
      ...
      <section className="mb-6">
        <div className="flex items-center justify-between mb-2.5">
          ...
          <button onClick={() => handleNavigate("/all-tools")} className="flex items-center gap-1 text-xs font-semibold active:scale-95" style={{ color: "hsl(43,82%,55%)" }}>All <ArrowRight size={12} /></button>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
          {recommendedTools.map((tool) => (
            <ToolRow key={tool.id} toolId={tool.id} onClick={() => handleOpen(tool.id, tool.path)} isFav={favorites.includes(tool.id)} onFav={() => toggleFavorite(tool.id)} />
          ))}
        </div>
      </section>

      {/* ── Browse All ──────────────────────────────────────────────────────── */}
      <button onClick={() => handleNavigate("/all-tools")} className="w-full py-4 rounded-2xl font-bold text-sm flex items-center justify-center gap-2 active:scale-[0.98] transition-transform" style={{ background: "hsl(218,44%,11%)", border: "1px solid hsl(218,38%,18%)", color: "hsl(43,25%,88%)" }}>
        <Grid3X3 size={15} />
      ...
      {/* ── Free User Teaser ────────────────────────────────────────────────── */}
      {!isPremium && (
        <div onClick={() => setLocation(pendingPremiumRequest ? "/pre-unlock" : "/premium-details")} className="mt-3 relative overflow-hidden p-6 rounded-[2.5rem] cursor-pointer active:scale-[0.98] transition-all group" style={{ background: "hsl(218,44%,11%)", border: "1px solid rgba(212,160,32,0.2)" }}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-white/70 mt-1 line-clamp-2">
                {pendingPremiumRequest ? "Finish your payment to activate Premium" : (freeUpgradeMessage || "Unlock professional features: unlimited client records, full measurement history, custom templates, and advanced tailoring tools.")}
              </p>
      ...
      {isPremium && proUpgradeMessage && (
        <div onClick={() => setShowProPopup(true)} className="mt-3 relative overflow-hidden p-6 rounded-[2.5rem] cursor-pointer active:scale-[0.98] transition-all group" style={{ background: "hsl(218,44%,11%)", border: "1px solid rgba(212,160,32,0.2)" }}>
          <div className="absolute -top-12 -right-12 w-32 h-32 rounded-full blur-3xl group-hover:opacity-80 transition-opacity" style={{ background: "rgba(212,160,32,0.08)" }} />
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center border group-hover:scale-110 transition-transform shrink-0" style={{ background: "rgba(212,160,32,0.12)", borderColor: "rgba(212,160,32,0.25)" }}>
              <Crown size={28} style={{ color: "hsl(43,82%,55%)" }} />
