...
import {
  ...
  Instagram, Facebook, Youtube, Lock, Wand2, Mail, Copy
} from "lucide-react";
...
export default function MeasurementCardGenerator() {
  ...
  // ── Derived data (must be BEFORE handlers that use it) ────────────────────
  ...
  // ── Handlers (now AFTER entries is defined) ──────────────────────────────
        ...
        {step === "customize" && selectedCustomer && selectedRecord && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                ...
                >
                  ...
                  <div className="relative w-full max-w-sm mx-4 animate-in zoom-in duration-200">
                    ...
                    <div className="rounded-2xl overflow-hidden shadow-2xl" style={{ background: sw.bg, color: isDark ? "#fff" : "#0f172a" }}>
                      ...
                      <div className="px-5 pt-3 pb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-2 flex-wrap text-[9px] font-bold" style={{ opacity: 0.5 }}>
                              {businessProfile?.phone && <span>{businessProfile.phone}</span>}
                              {socials?.whatsapp && <><span>·</span><span>{socials.whatsapp}</span></>}
                              {businessProfile?.email && <><span>·</span><span>{businessProfile.email}</span></>}
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      {previewStyle.id === "luxury" && (
                        <div className="flex items-center gap-3 px-5 py-1">
                          <div className="flex-1 h-px" style={{ background: `${sw.accent}40` }} />
                          <div className="w-1 h-1 rotate-45" style={{ background: sw.accent }} />
                          <div className="flex-1 h-px" style={{ background: `${sw.accent}40` }} />
                        </div>
                      )}

                      {/* Category / Label row */}
                      <div className={`px-5 py-2.5 border-t ${borderCls}`} style={previewStyle.id === "modern" ? { background: `${sw.accent}08` } : {}}>
                        <p className="text-[10px] font-black uppercase tracking-widest text-center" style={{ opacity: 0.6 }}>SHIRT — INITIAL MEASUREMENT</p>
                      </div>

                      {/* Minimal style: extra whitespace */}
                      {previewStyle.id === "minimal" && <div className="h-2" />}

                      {/* Customer row */}
                      <div className={`px-5 py-3 flex justify-between items-center ${previewStyle.id === "fashion" ? "border-y" : ""}`} style={previewStyle.id === "fashion" ? { borderColor: `${sw.accent}30` } : {}}>
                        <div>
                        ...
                        <p className="text-[9px] font-bold" style={{ opacity: 0.4 }}>{new Date().toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}</p>
                      </div>
                      ...
                      {/* Elegant: thinner / more opaque entries */}
                      {previewStyle.id === "elegant" && (
                        <div className="px-5 pb-3 text-center">
                          <span className="text-[8px] font-black uppercase tracking-[0.3em]" style={{ opacity: 0.25 }}>✦ PRECISION TAILORING ✦</span>
                        </div>
                      )}

                      {/* Footer */}
                      <div className={`px-5 py-3 border-t ${borderCls} text-center`}>
                        <p className="text-[8px] italic" style={{ opacity: 0.5 }}>All measurements in recorded unit. Confirm before cutting.</p>
                        <p className="text-[7px] font-bold uppercase tracking-widest mt-1.5" style={{ opacity: 0.2 }}>Generated by {businessProfile?.name || appName || "Your Brand"}</p>
                      </div>

                      {/* Fashion Studio: bottom accent border */}
                      {previewStyle.id === "fashion" && <div style={{ height: 2, background: sw.accent }} />}
                    </div>
            ...
            <div className="p-1 bg-slate-200 dark:bg-slate-800 rounded-[2rem] overflow-hidden shadow-2xl">
              {renderingCard ? (
                <div className={`w-full ${theme.bg} ${theme.text} p-12 flex items-center justify-center`}>
