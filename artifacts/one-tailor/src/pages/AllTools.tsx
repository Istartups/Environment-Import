...
import { Search, Star, X, Plus, Database, Ruler, Sparkles, Clock, ArrowRight, Grid3X3 } from "lucide-react";
...
export default function AllTools() {
  ...
  const suggestedTools = useMemo(() => {
    ...
    const popularFallback = ALL_TOOLS
      ...
      .slice(0, 8 - fromRecent.length);
    return [...fromRecent, ...popularFallback].slice(0, 8);
  ...
  const ToolRow = ({ tool, showCategory }: { tool: (typeof ALL_TOOLS)[number]; showCategory?: boolean }) => {
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {showCategory && catMeta && (
                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full shrink-0"
                  style={{ background: catMeta.bg, color: catMeta.color, border: `1px solid ${catMeta.borderColor}` }}>
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="px-4 pt-2 pb-24 space-y-4">
        ...
        {/* Recently Used (only when not searching, not in favs) */}
        {!query && activeFilter !== "fav" && recentToolObjects.length > 0 && (
          <div>
              ...
              {recentToolObjects.map((tool) => {
                ...
                return (
                  <button
                    ...
                    className="shrink-0 flex flex-col items-center gap-2 py-3 px-3 rounded-2xl active:scale-[0.95] transition-all"
                  ...
                  >
                    ...
                    <p className="text-[10px] font-black text-center leading-tight line-clamp-2 w-16" style={{ color: "hsl(43,25%,88%)" }}>
                      {tool.name}
                    </p>
                  </button>
                );
            ...
            {ORDERED_CATEGORIES.map((cat) => {
              ...
              return (
                <div key={cat}>
                  ...
                  {/* Clients quick-action shortcuts — show in both All and Clients view */}
                  {cat === "clients" && (
        ...
        {!query && activeFilter !== "all" && activeFilter !== "fav" && (
          <div className="space-y-2">
            {/* Client shortcuts in filtered view too */}
        ...
        {(query || activeFilter === "fav") && (
          <div className="space-y-2">
            {(!displayed || displayed.length === 0) ? (
              ...
              displayed.map((tool) => <ToolRow key={tool.id} tool={tool} showCategory={!!query} />)
            )}
          </div>
        )}
      </div>
