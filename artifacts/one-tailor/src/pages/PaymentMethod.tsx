...
import {
  ...
  ExternalLink, Check, ShieldCheck, Smartphone, Camera, ImageIcon,
  Clock, RefreshCw
} from "lucide-react";
                ...
                >
                  ...
                  <div className="text-left flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-600">Card</span>
                      <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-600">Transfer</span>
                      <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-600">USSD</span>
                    </div>
                  </div>
                  ...
                  <div className="text-left flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <span className="flex items-center gap-1 text-[9px] text-muted-foreground">
                        <Clock size={9} /> Verified in 2-4 hours
                      </span>
                    </div>
                  </div>
        ...
        {screen === "paystack_confirm" && (
          <motion.div key="paystack_confirm" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} className="space-y-6 text-center pt-6">
            ...
            <div className="flex items-center justify-center gap-3">
              <div className="flex flex-col items-center gap-1 p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/10">
                <CreditCard size={20} className="text-emerald-500" />
                <span className="text-[9px] font-bold text-emerald-600">Card</span>
              </div>
              <div className="flex flex-col items-center gap-1 p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/10">
                <Banknote size={20} className="text-emerald-500" />
                <span className="text-[9px] font-bold text-emerald-600">Transfer</span>
              </div>
              <div className="flex flex-col items-center gap-1 p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/10">
                <Smartphone size={20} className="text-emerald-500" />
                <span className="text-[9px] font-bold text-emerald-600">USSD</span>
              </div>
            </div>
          ...
          <motion.div key="manual" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} className="space-y-5">
            ...
            <div className="p-5 bg-primary/5 border border-primary/20 rounded-3xl space-y-4">
              ...
              <div className="space-y-2">
                {[
                  { label: "Bank Name",      value: settings?.bankName || "", field: "bankName" },
                  { label: "Account Number", value: settings?.accountNumber || "", field: "accountNumber" },
                  { label: "Account Name",   value: settings?.accountName || "", field: "accountName" },
                ].map(({ label, value, field }) => (
                  <button
                    key={field}
                    onClick={() => copyToClipboard(value, label)}
                    className="w-full flex justify-between items-center p-3.5 bg-card rounded-xl border border-border hover:border-primary/30 transition-colors text-left active:scale-[0.99]"
                  >
                    <div>
                      <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">{label}</p>
                      <p className="font-bold text-sm mt-0.5">{value || "—"}</p>
                    </div>
                    <Copy size={15} className="text-primary shrink-0 ml-2" />
                  </button>
                ))}
              </div>
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Transfer Reference (Optional)</label>
              <input
                type="text"
                placeholder="e.g. Your name or payment description"
                value={transferRef}
                onChange={e => setTransferRef(e.target.value)}
                className="w-full px-4 py-3 rounded-xl bg-card border border-border outline-none focus:border-primary text-sm"
              />
              <p className="text-[9px] text-muted-foreground">Helps us match your payment faster</p>
            </div>
            ...
            <div className="space-y-3">
              ...
              {!evidencePreview ? (
                <div className="grid grid-cols-2 gap-3">
                  <button
                    onClick={openGallery}
                    className="h-36 rounded-2xl border-2 border-dashed border-border flex flex-col items-center justify-center gap-2 bg-card hover:bg-muted/50 transition-all active:scale-[0.98]"
                  >
                    <ImageIcon className="text-muted-foreground" size={22} />
                  ...
                  >
                    ...
                    <p className="text-xs font-medium text-muted-foreground">Take Photo</p>
                  </button>
                  <input
            ...
            {/* Verification time note */}
            <div className="p-3 rounded-xl bg-blue-500/5 border border-blue-500/10 flex items-center gap-2">
              <Clock size={14} className="text-blue-500 shrink-0" />
              <p className="text-[10px] text-muted-foreground">
                Verification usually takes <span className="font-bold text-foreground">2-4 hours</span> during business hours. You'll receive a confirmation email once approved.
              </p>
            </div>
