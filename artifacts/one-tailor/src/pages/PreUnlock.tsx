...
import {
  ...
  AlertCircle, MapPin, Smartphone, Copy, SendHorizonal, ArrowLeft
} from "lucide-react";
...
export default function PreUnlock() {
  ...
  const [emailDebouncing, setEmailDebouncing]         = useState(false);
  ...
  const locationAlreadyFilled = !!(answers.city && answers.state);
  ...
  const checkEmailAvailability = async (email: string) => {
    ...
    finally  { setCheckingEmail(false); setEmailDebouncing(false); }
  };

  const handleEmailChange = (email: string) => {
    setAnswers(a => ({ ...a, email }));
    setEmailAvailable(null);
    setEmailDebouncing(true);
  ...
  const passwordStrength = getPasswordStrength(answers.password);
    ...
    <div className="max-w-xl mx-auto px-4 pb-20 pt-6 min-h-screen flex flex-col">
      ...
      {isQuestionStep && (
        <button
          ...
          className="mb-4 w-full flex items-center justify-between p-3 rounded-2xl bg-primary/5 border border-primary/10 text-primary hover:bg-primary/10 transition-colors"
        >
          <div className="flex items-center gap-2">
            <LogIn size={14} />
            <span className="text-xs font-bold">Already have an account?</span>
          </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {step === "client_count" && (
            <motion.div key="client_count" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }} className="space-y-6">
              ...
              <div className="pt-2">
                <button onClick={() => navigate("/home")} className="text-sm text-muted-foreground flex items-center gap-1">
          ...
          {step === "challenge" && (
            <motion.div key="challenge" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }} className="space-y-6">
              ...
              <div className="pt-2">
                <button onClick={() => setStep("client_count")} className="text-sm text-muted-foreground">← Back</button>
              </div>
            </motion.div>
          )}
          ...
          {step === "gender" && (
            <motion.div key="gender" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }} className="space-y-6">
              ...
              <div className="pt-2">
                <button onClick={() => setStep("challenge")} className="text-sm text-muted-foreground">← Back</button>
              </div>
            </motion.div>
          )}
            ...
            <motion.div key="location" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }} className="space-y-6">
              ...
              {locationAlreadyFilled && (
                <div className="p-3 rounded-xl bg-primary/5 border border-primary/10 flex items-center gap-2">
                  ...
                  <p className="text-xs text-muted-foreground">Pre-filled from your Brand Kit: <span className="font-bold text-foreground">{answers.city}, {answers.state}</span></p>
                </div>
              )}
          ...
          {step === "devices" && (
            <motion.div key="devices" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }} className="space-y-6">
              ...
              <div className="pt-2">
                <button onClick={() => setStep("location")} className="text-sm text-muted-foreground">← Back</button>
              </div>
            </motion.div>
          )}
                ...
                <div>
                  ...
                  <p className="text-sm text-foreground/80 leading-relaxed">
                    ...
                    {account?.rejectionReason && (
                      <span className="block mt-1 font-medium">Reason: {account.rejectionReason}</span>
                    )}
                  </p>
                </div>
              </div>
          ...
          {step === "success" && (
            <motion.div key="success" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center py-12 space-y-6">
              ...
              <button onClick={() => navigate("/message-center")} className="w-full py-4 bg-primary text-primary-foreground rounded-2xl font-bold text-lg">
