...
export default function AccountLogin() {
  ...
  const setPendingPremiumRequest = useAppStore(
    (s) => s.setPendingPremiumRequest,
  );
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user.isPremium) {
        ...
        toast({
          title: "Welcome back! ⭐",
          description: "Premium access restored.",
        });
        ...
        toast({
          title: "Logged in! 👋",
          description: "Welcome back to OneTailor.",
        });
      }
  ...
  const inputClass =
    "w-full pl-11 pr-4 py-3.5 rounded-2xl bg-card border border-border outline-none focus:border-primary transition-colors text-sm";
  const labelClass =
          ...
          {forgotSent ? (
            <div className="text-center space-y-4 pt-10">
              ...
              <p className="text-sm text-muted-foreground">
                If an account exists for <b>{forgotEmail}</b>, we've sent a
                reset link.
              </p>
      ...
      <div className="max-w-md mx-auto px-4 py-8 space-y-8">
        ...
        <div className="text-center space-y-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Log in to restore your premium access
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            >
              <div
                className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${rememberMe ? "bg-primary border-primary" : "border-border"}`}
              >
                {rememberMe && (
                  <Check
                    size={12}
