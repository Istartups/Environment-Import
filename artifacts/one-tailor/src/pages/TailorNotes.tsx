...
import {
  ...
  User, ImageIcon, Camera, ChevronDown, ChevronUp, CheckSquare, Square
} from "lucide-react";
...
interface NoteFormProps {
  ...
  onCancel: (hasContent: boolean) => void;
}
...
function NoteForm({ initialNote, customers, onSave, onCancel }: NoteFormProps) {
  ...
  const hasContent = title.trim() || content.trim() || tags.trim() || imageData;

  const handleImage = async (e: React.ChangeEvent<HTMLInputElement>, capture?: boolean) => {
  ...
  const handleBack = () => {
    if (hasContent && !initialNote) {
      if (!confirm("Discard this note? You have unsaved changes.")) return;
    }
    onCancel(hasContent);
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 animate-in fade-in duration-200">
      <div className="flex items-center gap-3 mb-2">
        <button type="button" onClick={handleBack} className="p-2 rounded-xl bg-muted/50 hover:bg-muted text-muted-foreground transition-all">
          <ChevronLeft size={18} />
        </button>
      ...
      <div>
        ...
        <input placeholder="Note title..." value={title} onChange={e => setTitle(e.target.value)} required className={inp} autoFocus />
      </div>

      <div>
        <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1 mb-1.5 block">Content *</label>
        <textarea placeholder="Write your note here..." value={content} onChange={e => setContent(e.target.value)} required rows={5} className={inp} />
      </div>

      {/* Image attachment */}
      <div>
        <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1 mb-1.5 block">Attach Image (optional)</label>
        {imageData ? (
          ...
          <div className="grid grid-cols-2 gap-2">
            <button type="button" onClick={openGallery} disabled={imageLoading}
              className="py-6 rounded-2xl border-2 border-dashed border-border hover:border-primary/40 flex flex-col items-center gap-2 text-muted-foreground hover:text-foreground transition-all active:scale-[0.98] disabled:opacity-50">
              {imageLoading ? (
            ...
            <button type="button" onClick={openCamera} disabled={imageLoading}
              className="py-6 rounded-2xl border-2 border-dashed border-border hover:border-primary/40 flex flex-col items-center gap-2 text-muted-foreground hover:text-foreground transition-all active:scale-[0.98] disabled:opacity-50">
        ...
        <input ref={fileRef} type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={(e) => handleImage(e)} />
      </div>
      ...
      <div>
        ...
        <select value={customerId ?? ""} onChange={e => setCustomerId(e.target.value ? parseInt(e.target.value) : null)} className={inp}>
      ...
      <div>
        ...
        <input placeholder="e.g. fabric, zipper, measurement..." value={tags} onChange={e => setTags(e.target.value)} className={inp} />
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 pt-2">
        <button type="button" onClick={handleBack}
          className="py-3.5 rounded-2xl border border-border text-sm font-bold text-muted-foreground hover:bg-muted transition-all">Cancel</button>
...
function NoteCard({ note, expanded, onToggleExpand, onEdit, onDelete, onTogglePin, onToggleArchive, onTagClick, onCustomerClick, isSelected, onToggleSelect, selectMode }: NoteCardProps) {
  const tags = parseTags(note.tags);
  const [imgExpanded, setImgExpanded] = useState(false);

  return (
    <div className={`bg-card border rounded-2xl overflow-hidden transition-all ${note.isPinned ? "border-primary/30 shadow-sm shadow-primary/10" : "border-border"} ${isSelected ? "ring-2 ring-primary border-primary" : ""}`}>
      {note.isPinned && (
        <div className="h-0.5 bg-gradient-to-r from-primary/60 to-primary/20" />
      )}

      {note.imageData && (
        <div className={`overflow-hidden cursor-pointer transition-all ${imgExpanded ? "max-h-96" : "max-h-32"}`} onClick={() => setImgExpanded(!imgExpanded)}>
          <img src={note.imageData} alt="Note attachment" className="w-full object-cover" />
      ...
      <div className="p-4 space-y-2" onClick={selectMode ? onToggleSelect : onToggleExpand}>
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {note.customerName && (
              <button
                onClick={(e) => { e.stopPropagation(); onCustomerClick(note.customerId!); }}
                className="flex items-center gap-1 mt-0.5 hover:text-primary transition-colors"
              >
        ...
        <p className={`text-xs text-muted-foreground leading-relaxed ${expanded ? "" : "line-clamp-3"}`}>{note.content}</p>
        {!expanded && note.content.length > 150 && (
          <button onClick={onToggleExpand} className="text-[10px] font-bold text-primary hover:underline">Read more</button>
        )}

        {/* Action buttons — always visible on mobile */}
        <div className="flex items-center gap-1 pt-1 flex-wrap">
          <button onClick={(e) => { e.stopPropagation(); onEdit(); }} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground transition-colors" title="Edit">
            <Edit2 size={11} />
          </button>
          <button onClick={(e) => { e.stopPropagation(); onTogglePin(); }} className={`p-1.5 rounded-lg transition-colors ${note.isPinned ? "bg-primary/10 text-primary" : "hover:bg-muted text-muted-foreground"}`} title={note.isPinned ? "Unpin" : "Pin"}>
            <Pin size={11} />
          </button>
          <button onClick={(e) => { e.stopPropagation(); onToggleArchive(); }} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground transition-colors" title={note.isArchived ? "Unarchive" : "Archive"}>
            {note.isArchived ? <RotateCcw size={11} /> : <Archive size={11} />}
          </button>
          <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="p-1.5 rounded-lg hover:bg-red-500/10 text-red-500 transition-colors" title="Delete">
            <Trash2 size={11} />
          </button>
        </div>
          ...
          <div className="flex flex-wrap gap-1 pt-1">
            {tags.map(t => (
              <button
                ...
                className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-muted text-muted-foreground border border-border hover:border-primary/40 hover:text-primary transition-colors"
              >
                {t}
              </button>
            ))}
...
export default function TailorNotes() {
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(`Delete ${selectedIds.size} note${selectedIds.size !== 1 ? "s" : ""}?`)) return;
    try {
      for (const id of selectedIds) {
        await fetch(`/api/notes/${id}?deviceId=${deviceId}`, { method: "DELETE" });
      }
      toast({ title: "Deleted", description: `${selectedIds.size} notes removed.` });
      setSelectedIds(new Set());
      setSelectMode(false);
      await fetchNotes();
    } catch { toast({ title: "Error", description: "Failed to delete some notes.", variant: "destructive" }); }
  };
    ...
    try {
      for (const id of selectedIds) {
        await fetch(`/api/notes/${id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ deviceId, isArchived: true }),
        });
      }
      toast({ title: "Archived", description: `${selectedIds.size} notes archived.` });
      setSelectedIds(new Set());
      setSelectMode(false);
      await fetchNotes();
    } catch { toast({ title: "Error", description: "Failed to archive some notes.", variant: "destructive" }); }
  };
  ...
  const handleCustomerClick = (customerId: number) => {
    setLocation(`/customer-measurement?customerId=${customerId}`);
  };
  ...
  const getEmptyStateMessage = () => {
    if (search) return "No matching notes.";
    switch (filter) {
      case "pinned": return "No pinned notes — tap the pin icon on a note to pin it.";
      case "archived": return "No archived notes.";
      case "general": return "No general notes.";
      case "client": return "No client notes.";
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="px-4 pb-24 space-y-4">
        ...
        {(view === "form" || view === "edit") && (
          <NoteForm
            ...
            customers={customers}
            onSave={handleSave}
            onCancel={(hasContent) => { setView("list"); setEditingNote(null); }}
          />
        )}
          ...
          <>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className={`shrink-0 px-3 py-3 rounded-xl text-[10px] font-bold border transition-all ${selectMode ? "bg-primary/10 border-primary text-primary" : "bg-card border-border text-muted-foreground hover:border-primary/30"}`}
              >
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
              {FILTERS.map(f => (
                <button
                  key={f.key}
                  onClick={() => { setFilter(f.key); exitSelectMode(); }}
            ...
            {loading ? (
              ...
              <div className="text-center py-20 bg-card border border-dashed border-border rounded-3xl space-y-3">
                ...
                <p className="text-sm font-bold text-muted-foreground">{getEmptyStateMessage()}</p>
                {filter === "all" && !search && (
      ...
      {selectMode && selectedIds.size > 0 && (
        <div className="fixed bottom-0 left-0 right-0 p-4 z-40 animate-in slide-in-from-bottom duration-200">
          <div className="max-w-xl mx-auto bg-card border border-primary/20 rounded-2xl p-4 shadow-2xl flex items-center justify-between">
            <p className="text-xs font-bold">{selectedIds.size} selected</p>
            <div className="flex gap-2">
              <button
                onClick={handleBulkArchive}
                className="px-4 py-2 rounded-xl bg-muted/50 border border-border text-xs font-bold text-muted-foreground hover:bg-muted transition-colors"
              >
                Archive
              </button>
              <button
                onClick={handleBulkDelete}
                className="px-4 py-2 rounded-xl bg-red-500/10 border border-red-500/20 text-red-500 text-xs font-bold hover:bg-red-500/20 transition-colors"
              >
                Delete
              </button>
            </div>
