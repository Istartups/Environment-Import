...
import { Crown, User, LogOut, LogIn, ChevronRight, ShieldCheck, Users, Ruler, FileText, Gift, Smartphone } from "lucide-react";
...
export default function Account() {
  ...
  const successfulInvites     = useAppStore((s) => s.successfulInvites);
  const selectedDeviceCount   = useAppStore((s) => s.selectedDeviceCount);

  const [, setLocation] = useLocation();
  const { toast }       = useToast();

  const [stats, setStats] = useState({ clients: 0, measurements: 0, quotes: 0 });

  useEffect(() => {
    const deviceId = getDeviceId();
    // Fetch stats
    fetch(`/api/tailoring/customers?deviceId=${deviceId}&limit=9999`)
      .then(r => r.json())
      .then(data => setStats(s => ({ ...s, clients: Array.isArray(data) ? data.length : 0 })))
      .catch(() => {});

    fetch(`/api/tailoring/measurements/recent?deviceId=${deviceId}&limit=9999`)
      .then(r => r.json())
      .then(data => setStats(s => ({ ...s, measurements: Array.isArray(data) ? data.length : 0 })))
      .catch(() => {});

    // Quotes from store
    const quotes = useAppStore.getState().fabricQuotes;
    if (quotes) setStats(s => ({ ...s, quotes: quotes.length }));
  }, []);
          ...
          <>
            ...
            <div className="bg-card border border-border rounded-3xl p-6 space-y-4">
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  className="text-[10px] font-bold text-primary px-3 py-1.5 rounded-xl bg-primary/10 hover:bg-primary/20 transition-colors"
                >
                  Edit
                </button>
              </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-card border border-border rounded-2xl p-4 text-center">
                ...
                <p className="text-lg font-black">{stats.clients}</p>
                <p className="text-[9px] text-muted-foreground uppercase font-bold">Clients</p>
              </div>
              <div className="bg-card border border-border rounded-2xl p-4 text-center">
                <Ruler size={16} className="text-emerald-500 mx-auto mb-1" />
                <p className="text-lg font-black">{stats.measurements}</p>
                <p className="text-[9px] text-muted-foreground uppercase font-bold">Measurements</p>
              </div>
              <div className="bg-card border border-border rounded-2xl p-4 text-center">
                <FileText size={16} className="text-violet-500 mx-auto mb-1" />
                <p className="text-lg font-black">{stats.quotes}</p>
                <p className="text-[9px] text-muted-foreground uppercase font-bold">Quotes</p>
              </div>
            </div>
            ...
            <div className="bg-card border border-border rounded-3xl overflow-hidden">
                ...
                >
                  ...
                  <div className="text-left flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {selectedDeviceCount} device{selectedDeviceCount !== 1 ? "s" : ""} · View details
                    </p>
                  </div>
              ...
              >
                ...
                <div className="text-left flex-1">
                  <p className="text-sm font-bold">Invite Tailors</p>
                  <p className="text-xs text-muted-foreground">
                    {successfulInvites > 0 ? `${successfulInvites} successful invite${successfulInvites !== 1 ? "s" : ""}` : "Earn rewards by referring others"}
                  </p>
                </div>
                <ChevronRight size={16} className="text-muted-foreground" />
              </button>
